...
def access_data():
    ...
    for record in records:
        if record[-3] != '':  # Skip empty strings
...
def clr_itemdata():
    ...
    clr_item = Button(root6, text="Clear", font=('Arial', 20), padx=100, pady=20, command=lambda: [
        clr_alldata(clr_variables), root6.destroy()])

    clr_item.grid(row=8, column=0, columnspan=4)
    root6.attributes("-topmost", True)

def clr_alldata(clr_variables=None):
    confirm = messagebox.askquestion("Confirmation", "Are you sure to delete all data?")
    if confirm == "yes":
        conn = sqlite3.connect('Restaurant_food_data.db')
        c = conn.cursor()
        if clr_variables is None:
            for i in range(len(foods)):
                c.execute("""UPDATE item SET No_of_customers=?, no_of_positives=?, no_of_negatives=?,
                            pos_perc=?, neg_perc=? WHERE oid=?""",
    ...
    for i in records:
        ...
        if rec[0] in selected_foods:
            ...
            c.execute("""UPDATE item SET item_name=:item_name, no_of_customers=:no_of_customers,
                        no_of_positives=:no_of_positives, no_of_negatives=:no_of_negatives,
                        pos_perc=:pos_perc, neg_perc=:neg_perc WHERE oid=:Oid""",
                      {
                          'item_name': rec[0],
